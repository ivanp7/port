...
{
    return fabs(value) < PORT_FLOAT_NEGLIGIBLE_NUM_EPS * PORT_FLOAT32_EPS;
}
...
{
    return fabs(value) < PORT_FLOAT_NEGLIGIBLE_NUM_EPS * PORT_FLOAT64_EPS;
}
...
port_float32_almost_equal_to_1(
        port_float32_t value)
{
    return port_float32_almost_equal_to_0(value - 1.0f);
}
...
port_float64_almost_equal_to_1(
        port_float64_t value)
{
    return port_float64_almost_equal_to_0(value - 1.0);
}
        ...
        port_float32_t value2)
{
    if (value1 == value2)
        ...
        return false;
    else if (copysign(1.0f, value1) != copysign(1.0f, value2))
        return port_float32_almost_equal_to_0(value1 - value2);

    value1 = fabs(value1);
    value2 = fabs(value2);

    port_float32_t minval = fmin(value1, value2);
    port_float32_t maxval = fmax(value1, value2);

    port_sint32_t exponent;
    maxval = frexp(maxval, &exponent) * 2.0f;
    exponent--; // mantissa is in range [1; 2)

    if (exponent <= 0)
        return port_float32_almost_equal_to_0(value1 - value2);

    minval *= exp2((port_float32_t)-exponent);

    return port_float32_almost_equal_to_0(maxval - minval);
}
        ...
        port_float64_t value2)
{
    if (value1 == value2)
        ...
        return false;
    else if (copysign(1.0, value1) != copysign(1.0, value2))
        return port_float64_almost_equal_to_0(value1 - value2);

    value1 = fabs(value1);
    value2 = fabs(value2);

    port_float64_t minval = fmin(value1, value2);
    port_float64_t maxval = fmax(value1, value2);

    port_sint32_t exponent;
    maxval = frexp(maxval, &exponent) * 2.0;
    exponent--; // mantissa is in range [1; 2)

    if (exponent <= 0)
        return port_float64_almost_equal_to_0(value1 - value2);

    minval *= exp2((port_float64_t)-exponent);

    return port_float64_almost_equal_to_0(maxval - minval);
}
